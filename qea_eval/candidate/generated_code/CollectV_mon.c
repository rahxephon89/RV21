...
static int handle_CollectV_queue(CollectVMonitor *mon) {
    int success = 1;
    int event;
    SMEDLValue *params;
    void *aux;
    ...
    while (pop_event(&mon->event_queue, &event, &params, &aux)) {
        switch (event) {
            case EVENT_CollectV_check:
                success = success &&
                    execute_CollectV_check(mon, params, aux);
                break;
            case EVENT_CollectV_result:
                success = success &&
                    execute_CollectV_result(mon, params, aux);
                break;
            case EVENT_CollectV_addV:
                success = success &&
                    execute_CollectV_addV(mon, params, aux);
                break;
        }
...
int execute_CollectV_addP(CollectVMonitor *mon, SMEDLValue *params, void *aux) {
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' handling imported event 'addP'\n");
#endif

    /* sce scenario */
    //if (!mon->ef.sce_flag) {
        switch (mon->sce_state) {
            case STATE_CollectV_sce_init:
                if (1) {
                    mon->s.pNum++;
                    {
                        ...
                        queue_CollectV_addV(mon, new_params, aux);
                    }

                    mon->sce_state = STATE_CollectV_sce_start;
                } else {
    ...
    return handle_CollectV_queue(mon);
}

int execute_CollectV_inRes(CollectVMonitor *mon, SMEDLValue *params, void *aux) {
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' handling imported event 'inRes'\n");
#endif

    /* sce1 scenario */
    //if (!mon->ef.sce1_flag) {
        switch (mon->sce1_state) {
            case STATE_CollectV_sce1_start:
                if (mon->s.pNum > 1) {
                    mon->s.res = mon->s.res + params[0].v.i;
                    mon->s.pNum--;

                    mon->sce1_state = STATE_CollectV_sce1_start;
                } else if (mon->s.pNum == 1) {
                    mon->s.res = mon->s.res + params[0].v.i;
                    {
                        SMEDLValue *new_params = malloc(sizeof(SMEDLValue) * 0);
                        if (new_params == NULL) {
                        ...
                        queue_CollectV_check(mon, new_params, aux);
                    }
                    mon->s.pNum--;
...
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' handling internal event 'check'\n");
#endif

    /* sce2 scenario */
    //if (!mon->ef.sce2_flag) {
        switch (mon->sce2_state) {
            case STATE_CollectV_sce2_start:
                if (mon->s.res > 0) {
                    {
                        ...
                        new_params[0].v.i = 1;
                        queue_CollectV_result(mon, new_params, aux);
                    }

                    mon->sce2_state = STATE_CollectV_sce2_start;
                } else if (mon->s.res == 0) {
                    {
                        SMEDLValue *new_params = malloc(sizeof(SMEDLValue) * 1);
                        if (new_params == NULL) {
...
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' queuing raised internal event 'check'\n");
#endif

    return push_event(&mon->event_queue, EVENT_CollectV_check, params, aux);
}
...
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' handling exported event 'result'\n");
#endif

    /* Export the event */
    return export_CollectV_result(mon, params, aux);
}

int queue_CollectV_result(CollectVMonitor *mon, SMEDLValue *params, void *aux) {
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' queuing raised exported event 'result'\n");
#endif

    return push_event(&mon->event_queue, EVENT_CollectV_result, params, aux);
}
...
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' handling exported event 'addV'\n");
#endif

    /* Export the event */
    return export_CollectV_addV(mon, params, aux);
}

int queue_CollectV_addV(CollectVMonitor *mon, SMEDLValue *params, void *aux) {
#if DEBUG >= 4
    fprintf(stderr, "Monitor 'CollectV' queuing raised exported event 'addV'\n");
#endif

    return push_event(&mon->event_queue, EVENT_CollectV_addV, params, aux);
}

int export_CollectV_addV(CollectVMonitor *mon, SMEDLValue *params, void *aux) {
    if (mon->callback_addV != NULL) {
        return (mon->callback_addV)(mon->identities, params, aux);
    }
    return 1;
}
...
CollectVMonitor * init_CollectV_monitor(SMEDLValue *identities) {
    CollectVState init_state;

    /* Initialize the monitor with default state */
    if (!default_CollectV_state(&init_state)) {
        /* malloc fail */
        return NULL;
    }
    CollectVMonitor *result = init_CollectV_with_state(identities, &init_state);
    if (result == NULL) {
 ...
 * needed.) default_CollectV_state() is aware of this, so unless changing
...
CollectVMonitor * init_CollectV_with_state(SMEDLValue *identities, CollectVState *init_state) {
    CollectVMonitor *mon = malloc(sizeof(CollectVMonitor));
    if (mon == NULL) {
        return NULL;
    }
